...
Resolve the current port from `.env` before using these examples:
...
curl -s http://localhost:$PORT/api/commander/sessions | jq

# Send input to a specific session
curl -s http://localhost:$PORT/api/commander/send-to-session \
  -H "Content-Type: application/json" \
...
```bash
# List all workspaces
curl -s http://localhost:$PORT/api/workspaces | jq

# Scan for available repos
curl -s http://localhost:$PORT/api/workspaces/scan-repos | jq

# Create a new worktree
curl -s http://localhost:$PORT/api/workspaces/create-worktree \
  -H "Content-Type: application/json" \
  -d '{"repoPath": "~/GitHub/games/monogame/zoo-game", "branchName": "feature/new-work"}'

# Remove a worktree
curl -s http://localhost:$PORT/api/workspaces/remove-worktree \
  -H "Content-Type: application/json" \
...
curl -s http://localhost:$PORT/api/greenfield/templates | jq

# Create new project
curl -s http://localhost:$PORT/api/greenfield/create \
  -H "Content-Type: application/json" \
...
# Check for updates
curl -s http://localhost:$PORT/api/git/check-updates | jq

# Pull updates
curl -s http://localhost:$PORT/api/git/pull -X POST
```
...
curl -s http://localhost:$PORT/api/quick-links | jq

# Get recent sessions
curl -s http://localhost:$PORT/api/quick-links/recent-sessions | jq
```
...
curl -s http://localhost:$PORT/api/continuity/ledger | jq

# Get workspace continuity info
curl -s http://localhost:$PORT/api/continuity/workspace | jq
```
...
curl -s http://localhost:$PORT/api/user-settings | jq

# Update global settings
curl -s http://localhost:$PORT/api/user-settings/global \
...
curl -s http://localhost:$PORT/api/ports | jq
```
...
```bash
for sid in $(curl -s http://localhost:$PORT/api/commander/sessions | jq -r '.sessions[] | select(.id | contains("claude")) | .id'); do
  curl -s http://localhost:$PORT/api/commander/send-to-session \
    -H "Content-Type: application/json" \
...
```bash
curl -s http://localhost:$PORT/api/commander/sessions | jq '.sessions[] | {id, status, branch}'
```
