...
function buildRunbook({
  ...
  lines.push('');
  lines.push('```bash');
  lines.push(`git clone --mirror "${repoUrl || '<repo-url>'}" claude-orchestrator.mirror.git`);
  lines.push('tar -czf claude-orchestrator.mirror.git.tgz claude-orchestrator.mirror.git');
  lines.push('```');
  lines.push('');
  lines.push('### 2) Fresh rewrite clone');
  lines.push('');
  lines.push('```bash');
  lines.push(`git clone "${repoUrl || '<repo-url>'}" claude-orchestrator-rewrite`);
  lines.push('cd claude-orchestrator-rewrite');
